...
from app.modules.product.models import Product, ProductCategoryLink, ProductIngredientLink
...
def seed_categories(db: Session) -> dict[str, Category]:
    ...
    padres = [
        {"name": "Hamburguesas",     "description": "Hamburguesas artesanales",           "order_display": 1},
        {"name": "Bebidas",          "description": "Bebidas frías",                       "order_display": 2},
        {"name": "Acompañamientos",  "description": "Para complementar tu pedido",         "order_display": 3},
    ...
    subcats = [
        {"name": "Clásicas",    "description": "Las favoritas de siempre",       "order_display": 1, "parent_id": cats["Hamburguesas"].id},
        {"name": "Premium",     "description": "Ingredientes seleccionados",      "order_display": 2, "parent_id": cats["Hamburguesas"].id},
        {"name": "Sin Alcohol", "description": "Refrescos, jugos y agua",         "order_display": 1, "parent_id": cats["Bebidas"].id},
    ]
    for data in subcats:
        obj = db.exec(select(Category).where(Category.name == data["name"])).first()
        if not obj:
...
def seed_ingredients(db: Session) -> dict[str, Ingredient]:
    ...
    data = [
        {"name": "Pan brioche",      "description": "Pan suave y esponjoso",         "is_allergen": True},
        {"name": "Carne vacuna",     "description": "Medallón de carne 200g",         "is_allergen": False},
        {"name": "Lechuga",          "description": "Lechuga fresca",                 "is_allergen": False},
        {"name": "Tomate",           "description": "Tomate fresco en rodajas",       "is_allergen": False},
        {"name": "Cebolla",          "description": "Cebolla morada",                 "is_allergen": False},
        {"name": "Queso cheddar",    "description": "Queso cheddar fundido",          "is_allergen": True},
        ...
        {"name": "Jalapeños",        "description": "Jalapeños encurtidos",           "is_allergen": False},
...
def _link_categories(db: Session, product: Product, categories: list[tuple[Category, bool]]) -> None:
    ...
    for cat, is_primary in categories:
        ...
        if not exists:
            db.add(ProductCategoryLink(product_id=product.id, category_id=cat.id, is_primary=is_primary))


def _link_ingredients(db: Session, product: Product, ingredients: list[tuple[Ingredient, bool]]) -> None:
    ...
    for ingr, is_removable in ingredients:
        ...
        if not exists:
            db.add(ProductIngredientLink(product_id=product.id, ingredient_id=ingr.id, is_removable=is_removable))


def seed_products(db: Session, cats: dict[str, Category], ingrs: dict[str, Ingredient]) -> None:
    productos = [
        {
            "data": {"name": "Hamburguesa Clásica", "description": "La clásica de siempre: carne, lechuga, tomate y mayonesa", "base_price": Decimal("1500.00"), "stock_quantity": 50, "prep_time_min": 15},
            "cats": [("Clásicas", True), ("Hamburguesas", False)],
            "ingrs": [("Pan brioche", False), ("Carne vacuna", False), ("Lechuga", True), ("Tomate", True), ("Cebolla", True), ("Mayonesa", True)],
        },
        {
            "data": {"name": "Hamburguesa BBQ Bacon", "description": "Carne jugosa, bacon crocante y salsa BBQ ahumada", "base_price": Decimal("2000.00"), "stock_quantity": 30, "prep_time_min": 20},
            "cats": [("Premium", True), ("Hamburguesas", False)],
            "ingrs": [("Pan brioche", False), ("Carne vacuna", False), ("Queso cheddar", True), ("Bacon", True), ("Salsa BBQ", True), ("Cebolla", True)],
        },
        {
            "data": {"name": "Hamburguesa Doble", "description": "Doble medallón de carne con queso cheddar", "base_price": Decimal("2200.00"), "stock_quantity": 25, "prep_time_min": 20},
            "cats": [("Clásicas", True), ("Hamburguesas", False)],
            "ingrs": [("Pan brioche", False), ("Carne vacuna", False), ("Queso cheddar", True), ("Lechuga", True), ("Tomate", True), ("Mayonesa", True)],
        },
        {
            "data": {"name": "Hamburguesa Picante", "description": "Para los que la quieren con fuego: jalapeños y salsa BBQ", "base_price": Decimal("1900.00"), "stock_quantity": 20, "prep_time_min": 18},
            "cats": [("Premium", True), ("Hamburguesas", False)],
            "ingrs": [("Pan brioche", False), ("Carne vacuna", False), ("Jalapeños", True), ("Queso cheddar", True), ("Salsa BBQ", True), ("Mayonesa", True)],
        },
        {
            "data": {"name": "Papas Fritas", "description": "Papas fritas crocantes con sal", "base_price": Decimal("800.00"), "stock_quantity": 100, "prep_time_min": 10},
            "cats": [("Acompañamientos", True)],
            "ingrs": [("Papas", False)],
        },
        {
            "data": {"name": "Aros de Cebolla", "description": "Cebolla rebozada y frita", "base_price": Decimal("900.00"), "stock_quantity": 80, "prep_time_min": 12},
            "cats": [("Acompañamientos", True)],
            "ingrs": [("Cebolla", False)],
        },
        {
            "data": {"name": "Coca Cola 500ml", "description": "Bebida gaseosa fría", "base_price": Decimal("600.00"), "stock_quantity": 150, "prep_time_min": 1},
            "cats": [("Sin Alcohol", True), ("Bebidas", False)],
            "ingrs": [],
        },
        {
            "data": {"name": "Agua Mineral 500ml", "description": "Agua mineral sin gas", "base_price": Decimal("400.00"), "stock_quantity": 200, "prep_time_min": 1},
            "cats": [("Sin Alcohol", True), ("Bebidas", False)],
            "ingrs": [],
        },
        {
            "data": {"name": "Brownie de Chocolate", "description": "Brownie casero con chips de chocolate", "base_price": Decimal("700.00"), "stock_quantity": 40, "prep_time_min": 5},
            "cats": [("Postres", True)],
            "ingrs": [],
        },
    ]
    ...
    for p in productos:
        product = db.exec(select(Product).where(Product.name == p["data"]["name"])).first()
        if not product:
