...
def admin_list_pedidos(
    _: Annotated[User, Depends(require_pedido_staff)],
    svc: PedidoService = Depends(get_pedido_service),
    estado_id: Optional[int] = Query(
        default=None, description="Filtrar por ID de estado"
    ),
    usuario_id: Optional[int] = Query(
        default=None, description="Filtrar por ID de usuario"
    ),
    ...
    pedidos = svc.get_all_pedidos_admin(
        estado_id=estado_id, usuario_id=usuario_id, offset=offset, limit=limit
    )
...
def list_mis_pedidos(
    ...
    limit: int = Query(
        default=20, ge=1, le=100, description="Límite de resultados (máx 100)"
    ),
) -> PedidoList:
    """LISTADO PARA CLIENTE: Solo ve sus propios pedidos."""
    pedidos = svc.get_pedidos_by_usuario(
        usuario_id=current_user.id, offset=offset, limit=limit
    )
...
def update_pedido(
    ...
    return svc.update_pedido(
        pedido_id=pedido_id, pedido_in=pedido_in, usuario_id=current_user.id
    )
