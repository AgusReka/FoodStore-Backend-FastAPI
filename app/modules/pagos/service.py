...
class FormaPagoService:
    ...
    def get_all(
        self, offset: int = 0, limit: int = 20, include_deleted: bool = False
    ) -> FormaPagoList:
        with PaymentUnitOfWork(self._session) as uow:
            formas_pago = list(
                uow.formas_pago.get_all(
                    offset=offset, limit=limit, include_deleted=include_deleted
                )
            )
    ...
    def create_forma_pago(
        self, forma_pago_in: FormaPagoCreate, es_admin: bool = True
    ) -> FormaPago:
        if not es_admin:
            raise AuthorizationError(
                message="No tienes permisos para crear formas de pago."
            )
        with PaymentUnitOfWork(self._session) as uow:
    ...
    def update_forma_pago(
        self, forma_pago_id: int, forma_pago_in: FormaPagoUpdate, es_admin: bool = True
    ) -> FormaPago:
        if not es_admin:
            raise AuthorizationError(
                message="Solo administradores pueden modificar formas de pago"
            )
        with PaymentUnitOfWork(self._session) as uow:
            forma_pago = uow.formas_pago.get(forma_pago_id)
            if not forma_pago or forma_pago.deleted_at is not None:
    ...
    def delete_forma_pago(self, forma_pago_id: int, es_admin: bool = True) -> None:
        if not es_admin:
            raise AuthorizationError(
                message="Solo administradores pueden eliminar formas de pago"
            )
        with PaymentUnitOfWork(self._session) as uow:
            forma_pago = uow.formas_pago.get(forma_pago_id)
            if not forma_pago or forma_pago.deleted_at is not None:
...
class PaymentService:
    ...
    def crear_preferencia(
        ...
        with PaymentUnitOfWork(self._session) as uow:
            ...
            if ingredient_requirements:
                ingredients = uow.ingredients.get_all_in(
                    list(ingredient_requirements.keys())
                )
                missing = set(ingredient_requirements.keys()) - {
            ...
            # 3. Calcular total
            costo_envio = (
                COSTO_ENVIO_DELIVERY
                if req.tipo_envio == "delivery"
                else Decimal("0.00")
            )
    ...
    def confirmar_pago(self, payment_id: int) -> ConfirmarPagoResponse:
        self._check_sdk()
        ...
        with PaymentUnitOfWork(self._session) as uow:
            ...
            for item in checkout_data["items"]:
                ...
                detalle = DetallePedido(
                    ...
                    producto_precio_unitario=producto.base_price
                    if producto
                    else Decimal("0"),
                    cantidad=item["cantidad"],
                    subtotal=(producto.base_price if producto else Decimal("0"))
                    * item["cantidad"],
                )
